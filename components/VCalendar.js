...
export class VCalendar extends React.Component {
    constructor(props) {
        ...
        this.eventsGridScrollX = new Animated.Value(2);
    ...
    yToHour = (y) => {
        ...
        var ret = {}
        if (prac < 6) {
            ret = { 'from': `${hour}:00`, 'to': `${hour}:30` }
        } else {
            ret = { 'from': `${hour}:30`, 'to': `${hour + 1}:00` }
        }
            ...
            userEvents.push(
                <TouchableOpacity key={`userEvents${index}`} onPress={bind(this.onClickEvent, this, item)} id={index}
                    style={[{
                        ...
                        backgroundColor: item.color, opacity: 0.9, color: '#615B73',
